...
- `InspectorViewModel` / 节点树：~~可作为 AI 视觉/界面理解上下文，但需要单独授权和裁剪~~。**已撤回（见 §10.2 不可妥协的边界）**：AI 链路最终不实例化 `FloatingInspector` 也不写 `InspectorViewModel`，避免污染用户当前 Inspector 状态；只读 `rootInActiveWindow + freeze()` 这一最小数据源。屏幕感知走 `ScreenSnapshotProvider` + `AiNodeTreeCompactor`。
...
- **agent / 经验本 / 草稿生成 三模块解耦**（2026-05-13 落地）：
  - agent 执行模块（`AiAgentSession`）每步**自动**沉淀到经验本；agent 自身不感知草稿
  - 经验本（`AiAgentExperienceBook`）作为纯数据层，三个出口：`recall` 回喂 prompt / `queryAll/loadEntry` 给 UI / `convertToDraft` 给草稿入口
  - 草稿生成（`ExperienceToTaskConverter`）是纯工具类，**完全由用户在 UI 主动点击触发**，把成功经验里 `convertible` 的 step 翻译成 `XTask` 弹 `FlowEditor`
  - 关键：这**不是**"自动 vs 手动落库"的开关关系，是模块边界
  - 会话结果通过独立通知 channel `ai_agent_outcome` 告知用户
  - 详见 `aidoc/20-experience-book-design.md`
